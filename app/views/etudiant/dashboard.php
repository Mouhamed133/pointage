...
<head>
  ...
  <style>
    * {
    ...
    .scan-confirm {
      animation: scan-flash .5s ease forwards
    }
...
<body>
        ...
        <div class="scan-confirm mb-5 p-4 rounded-2xl flex items-center gap-4" style="background:<?= $s['bg'] ?>;border:1px solid <?= $s['border'] ?>">
          ...
          <div>
            ...
            <p class="text-xs mt-0.5" style="color:<?= $s['icon'] ?>">
              ...
              <?php elseif ($scanType === 'erreur'): ?>Pointage refuse — consultez l'administration ou verifiez votre emploi du temps
        ...
        <?php if ($pointageAujourdhui): ?>
          <div class="card p-4 mb-6 flex items-center gap-4" style="border-left:3px solid #34d399">
            <div class="w-10 h-10 bg-emerald-900 rounded-xl flex items-center justify-center">
              <svg viewBox="0 0 24 24" fill="none" stroke="#34d399" stroke-width="2.5" class="w-5 h-5">
                <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <div>
        ...
        <?php else: ?>
          <div class="card p-4 mb-6 flex items-center gap-4" style="border-left:3px solid #f87171">
            <div class="w-10 h-10 bg-red-900 rounded-xl flex items-center justify-center">
              <svg viewBox="0 0 24 24" fill="none" stroke="#f87171" stroke-width="2.5" class="w-5 h-5">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 p-4">
            ...
            <?php else: ?>
              <?php foreach (array_slice($historique, 0, 5) as $h): ?>
                <?php $badges = ['present' => 'badge-present', 'retard' => 'badge-retard', 'absence' => 'badge-absent'];
        ...
        <div class="max-w-lg mx-auto space-y-4">
          <div id="geo-status" class="card p-4 flex items-center gap-3" style="border-left:3px solid #60a5fa">
            <div class="w-9 h-9 bg-blue-900/50 rounded-xl flex items-center justify-center flex-shrink-0">
              <svg viewBox="0 0 24 24" fill="none" stroke="#60a5fa" stroke-width="2" class="w-5 h-5">
  ...
  <script>
    ...
    function telechargerQR() {
      var c = document.getElementById('qr-grand');
      if (!c) {
        showToast('QR non trouve', 'error');
        return;
      }
      var canvas = c.querySelector('canvas');
      if (canvas) {
        var a = document.createElement('a');
        a.download = 'qrcode-<?= htmlspecialchars($nom) ?>.png';
        a.href = canvas.toDataURL('image/png');
        a.click();
        showToast('Telecharge !', 'success');
      } else {
        showToast('QR non genere', 'error');
      }
    }

    function regenererMonQR() {
      if (!confirm('Regenerer votre QR Code ? L\'ancien ne fonctionnera plus.')) return;
      fetch('index.php?route=etudiant/qr/regenerer', {
          method: 'POST'
        })
        .then(function(r) {
          return r.json();
    ...
    function uploadPhoto() {
      ...
      if (!input.files || !input.files[0]) {
        showToast('Selectionnez une photo', 'error');
        return;
      }
      ...
      btn.disabled = true;
      fetch('index.php?route=etudiant/profil/photo', {
          method: 'POST',
          body: formData
        })
        .then(function(r) {
    ...
    function changerMotDePasse() {
      ...
      if (nouveau.length < 6) {
        showToast('Le nouveau mot de passe doit avoir au moins 6 caracteres', 'error');
        return;
      }
      if (nouveau !== confirm) {
    ...
    // ABSENCES
    // ============================================
    function ouvrirFormulaireAbsence() {
      var f = document.getElementById('form-absence');
      if (f) {
    ...
    function fermerFormulaireAbsence() {
      var f = document.getElementById('form-absence');
    ...
    function changerTypeAbsence(type) {
      ...
      if (type === 'absence_passee') {
        dateLabel.textContent = 'Date de l\'absence *';
        dateHint.textContent = 'Choisissez la date ou vous etiez absent';
        dateInput.max = today;
        dateInput.min = '';
        dateInput.value = '';
        docSection.style.display = 'block';
        infoBox.style.display = 'block';
        infoBox.style.color = '#60a5fa';
        infoBox.textContent = 'Vous justifiez une absence passee. Joignez un document si possible.';
      } else if (type === 'absence_future') {
        dateLabel.textContent = 'Date de l\'absence prevue *';
        dateHint.textContent = 'Choisissez la date a venir';
    ...
    // ============================================
    var ECOLE_LAT = 14.679620,
      ECOLE_LNG = -17.441229,
      RAYON_MAX = 500;
    var positionOK = false;
    var currentGeoLat = null,
      currentGeoLng = null;
    var scannerEcole = null,
      scanEcoleActif = false,
      lastScanEcole = 0;
    var scannerPerso = null,
      scanPersoActif = false,
      lastScanPerso = 0;
    ...
    function initPointer() {
      ...
      var statusBox = document.getElementById('geo-status'),
        statusText = document.getElementById('geo-status-text'),
        distText = document.getElementById('geo-distance-text');
      ...
      fetch('index.php?route=school/config')
        ...
        .then(function(res) {
          if (res.success && res.data) {
            ...
            var el = document.getElementById('rayon-display');
            if (el) el.textContent = RAYON_MAX + ' metres';
          }
    ...
    function _demarrerGPS(statusBox, statusText, distText) {
      statusText.textContent = 'Verification de votre position...';
      distText.textContent = 'En attente GPS';
      if (!navigator.geolocation) {
        statusBox.style.borderLeftColor = '#f87171';
        statusText.textContent = 'Geolocalisation non supportee';
        distText.textContent = 'Votre navigateur ne supporte pas le GPS';
        return;
      }
      navigator.geolocation.getCurrentPosition(
        function(pos) {
          ...
          if (dist <= RAYON_MAX) {
            ...
            statusBox.style.borderLeftColor = '#34d399';
            statusText.textContent = 'Position validee — vous etes sur place';
            distText.textContent = 'Distance : ' + dist + 'm de l\'etablissement';
            setStatus('scan-status-ecole', 'Position OK — Activez la camera', 'success');
            setStatus('scan-status-perso', 'Position OK — Activez la camera', 'success');
          } else {
            positionOK = false;
            statusBox.style.borderLeftColor = '#f87171';
            statusText.textContent = 'Hors du perimetre autorise';
            distText.textContent = 'Distance : ' + dist + 'm (max ' + RAYON_MAX + 'm) — Rapprochez-vous';
            setStatus('scan-status-ecole', 'Hors perimetre — pointage impossible (' + dist + 'm)', 'error');
            setStatus('scan-status-perso', 'Hors perimetre — pointage impossible (' + dist + 'm)', 'error');
          }
        },
        function(err) {
          statusBox.style.borderLeftColor = '#fbbf24';
          statusText.textContent = 'Position indisponible';
    ...
    function switchTab(tab) {
      if (scanEcoleActif) arreterScanEcole();
      if (scanPersoActif) arreterScanPerso();
    ...
    function setStatus(elId, msg, type) {
      ...
      el.textContent = msg;
      var s = {
        info: 'background:#1a2333;color:#60a5fa;border:1px solid #1e3a5f',
        success: 'background:#0d2a1e;color:#34d399;border:1px solid #064e3b',
        error: 'background:#2d1414;color:#f87171;border:1px solid #7f1d1d',
        warning: 'background:#2d2006;color:#fbbf24;border:1px solid #78350f'
      };
    ...
    function demarrerScanEcole() {
      if (!positionOK) {
        showToast('Hors perimetre — pointage refuse', 'error');
        return;
      }
      ...
      scannerEcole.start({
          ...
          var now = Date.now();
          if (now - lastScanEcole < 3000) return;
          lastScanEcole = now;
          setStatus('scan-status-ecole', 'QR detecte ! Traitement...', 'info');
          envoyerViaQrEcole(decoded);
        }, function() {})
        .then(function() {
          scanEcoleActif = true;
          document.getElementById('btn-start-ecole').style.display = 'none';
          document.getElementById('btn-stop-ecole').style.display = 'block';
          setStatus('scan-status-ecole', 'Camera active — Scannez le QR de l\'ecole', 'success');
        })
        .catch(function() {
          setStatus('scan-status-ecole', 'Impossible d\'acceder a la camera', 'error');
        });
    }

    function arreterScanEcole() {
      if (!scannerEcole || !scanEcoleActif) return;
      scannerEcole.stop().then(function() {
        scanEcoleActif = false;
        document.getElementById('btn-start-ecole').style.display = 'block';
        document.getElementById('btn-stop-ecole').style.display = 'none';
        document.getElementById('etu-qr-reader-ecole').innerHTML = '';
        setStatus('scan-status-ecole', 'Camera arretee', 'info');
      });
    }

    function envoyerViaQrEcole(tokenEcole) {
      var body = 'token_ecole=' + encodeURIComponent(tokenEcole);
      if (currentGeoLat !== null && currentGeoLng !== null) {
        body += '&lat=' + encodeURIComponent(currentGeoLat) + '&lng=' + encodeURIComponent(currentGeoLng);
      }
      fetch('index.php?route=school/pointer', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded'
          },
        ...
        .then(function(res) {
          if (res.success) {
            ...
            setStatus('scan-status-ecole', libelle + ' enregistree !', 'success');
            showToast(res.message || 'Pointage enregistre !', 'success');
            arreterScanEcole();
            setTimeout(function() {
              location.reload();
            }, 2000);
          } else {
            setStatus('scan-status-ecole', 'Erreur : ' + res.message, 'error');
    ...
    function demarrerScanPerso() {
      ...
      scannerPerso.start({
          ...
          var now = Date.now();
          if (now - lastScanPerso < 3000) return;
          lastScanPerso = now;
          setStatus('scan-status-perso', 'QR detecte ! Traitement...', 'info');
          envoyerViaQrPerso(decoded);
        }, function() {})
        .then(function() {
          scanPersoActif = true;
          document.getElementById('btn-start-perso').style.display = 'none';
          document.getElementById('btn-stop-perso').style.display = 'block';
          setStatus('scan-status-perso', 'Camera active — Scannez votre QR Code', 'success');
        })
        .catch(function() {
          setStatus('scan-status-perso', 'Impossible d\'acceder a la camera', 'error');
        });
    }

    function arreterScanPerso() {
      if (!scannerPerso || !scanPersoActif) return;
      scannerPerso.stop().then(function() {
        scanPersoActif = false;
        document.getElementById('btn-start-perso').style.display = 'block';
        document.getElementById('btn-stop-perso').style.display = 'none';
        document.getElementById('etu-qr-reader-perso').innerHTML = '';
        setStatus('scan-status-perso', 'Camera arretee', 'info');
      });
    }
    ...
    function envoyerViaQrPerso(token) {
      fetch('index.php?route=presences/pointer', {
        ...
        .then(function(res) {
          if (res.success) {
            var libelle = res.data && res.data.mode === 'depart' ? 'Depart' : 'Arrivee';
            setStatus('scan-status-perso', libelle + ' enregistree !', 'success');
            showToast(res.message || 'Pointage enregistre !', 'success');
            arreterScanPerso();
            setTimeout(function() {
              location.reload();
            ...
            setStatus('scan-status-perso', 'Erreur : ' + res.message, 'error');
            showToast(res.message, 'error');
          }
        }).catch(function() {
          showToast('Erreur reseau', 'error');
